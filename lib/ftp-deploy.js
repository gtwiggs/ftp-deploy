...
module.exports = {
   ...
   */
  buildManifest: function(path, remoteDirectory) {
    return this._buildManifest(path, remoteDirectory, path).sort((a, b) => {
      return ('' + a.path).localeCompare(b.path);
    })
  },

  _buildManifest: function(path, remoteDirectory, root) {
    var manifest = [];
    fileSystem.recurseSync(path, function(
      ...
      if (!filename) {
        manifest = manifest.concat(
          // TODO: sort out the structural issues requiring referencing the exports:
          module.exports._buildManifest(filepath, remoteDirectory, root)
        );
      }
    });
    return manifest;
